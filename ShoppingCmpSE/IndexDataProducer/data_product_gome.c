...
int g_gome_mobile_product_stop;
...
static void* product_mobile_data(void* p)
{
    ...
    tsk.direction = RIGHT;
    tsk.url = HOME_PAGE_GOME;

    ret = parse_page_by_url(&tsk, buf_url_mobile_phone, URL_LEN);
    if (-1 == ret)
    {
        fprintf(stderr, "在gome主页中获取'手机、摄影、数码'的url出错...\n");
        g_gome_mobile_product_stop = 1;
        return NULL;
    }

    // step2 在'手机、摄影、数码'页面中找‘手机’的url
	init_parse_task(&tsk);
    tsk.find_flag = 1;
    tsk.flag_str = "<dt>手机通讯</dt>";
	...
	if (-1 == ret)
	{
		fprintf(stderr, "在'手机、摄影、数码'中获取‘手机’的url出错...\n");
		g_gome_mobile_product_stop = 1;
		return NULL;
	}
	...
	while (!g_gome_mobile_product_stop)
	{
		char* find_slash = NULL;
		tsk.url = buf_url_cur;
